...
class PriorResnetBlock(nn.Module):
    """
    Residual block with timestep conditioning.
    ...
    def __init__(
        ...
        if self.ssl_cond:
            self.lin = nn.Linear(ssl_dim, out_channels)
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        if self.ssl_cond:
            return self.skip_connection(x) + h + self.lin(context).unsqueeze(-1).unsqueeze(-1)
        else:
            return self.skip_connection(x) + h
...
class DownBlock(nn.Module):
    ...
    def forward(
        ...
        for resnet in self.resnets:
            hidden_states = resnet(hidden_states, temb, context=context[1])
...
class AttnDownBlock(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            hidden_states = resnet(hidden_states, temb, context[1])
...
class CrossAttnDownBlock(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            hidden_states = resnet(hidden_states, temb, context=context[1])
            hidden_states = attn(hidden_states, context=context[0])
            output_states.append(hidden_states)
...
class AttnMidBlock(nn.Module):
    ...
    def __init__(
        ...
        self.resnet_1 = PriorResnetBlock(
            spatial_dims=spatial_dims,
            in_channels=in_channels,
            out_channels=in_channels,
            temb_channels=temb_channels,
        ...
        self.resnet_2 = PriorResnetBlock(
            spatial_dims=spatial_dims,
            in_channels=in_channels,
            out_channels=in_channels,
            temb_channels=temb_channels,
    ...
    def forward(
        ...
        hidden_states = self.resnet_1(hidden_states, temb, contex=context[1])
        hidden_states = self.attention(hidden_states)
        hidden_states = self.resnet_2(hidden_states, temb, contex=context[1])
...
class CrossAttnMidBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.resnet_1(hidden_states, temb, context=context[1])
        hidden_states = self.attention(hidden_states, context=context[0])
        hidden_states = self.resnet_2(hidden_states, temb, context=context[1])
